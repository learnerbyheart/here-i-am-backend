...
describe('Test suite for the here i am service', () => {
    ...
    describe('#saveLocation', () => {
        it('should save the location', () => {
            ...
            expect(actual).toStrictEqual({ latitude: 10.23404, longitude: 20.3312312, time: now });
        })

        it('should use the current date when no time is given', () => {
        ...
        it('should save the location when latitude is -85', () => {
            ...
            expect(actual).toStrictEqual({ latitude: -85, longitude: 23.1, time: now })
        });
        ...
        it('should save the location when longitude is -180', () => {
            ...
            expect(actual).toStrictEqual({ latitude: 21.21, longitude: -180, time: now })
        });
        ...
        it('should throw an error when time is of type string', () => {
            expect(() => hereIAmService.saveLocation(23.1, 22, "Test")).toThrow()
        })

        it('should throw an error when time is null', () => {
    ...
    describe('#getLocation', () => {
        it('should get the saved location', () => {
            ...
            const time = new Date()
            hereIAmService.saveLocation(latitude, longitude, time)
            expect(hereIAmService.getLocation()).toStrictEqual({ latitude, longitude, time })
        });
        ...
        it('should get null values for latitude, longitude and time when no location is saved', () => {
            expect(hereIAmService.getLocation()).toStrictEqual({ latitude: null, longitude: null, time: null })
        })
    })
})